...
export default tool({
  description: "Write data to a file, optionally at a specific offset and length",
  parameters: z.object({
    ...
    offset: z.number().optional().describe("The offset at which to start writing (optional)"),
    length: z.number().optional().describe("The number of bytes to write (optional)"),
  }),
  execute: async ({ fileName, content, offset, length }) => {
    ...
    if (offset !== undefined && length !== undefined) {
      ...
      const newContent = existingContent.substring(0, start) + content + existingContent.substring(end);
